...
Note the [IDF modifier](/documentation/manage-data/indexing/#idf-modifier), which configures the sparse vector for queries that use the inverse document frequency (IDF). By default, IDF statistics are computed across the whole collection; see [Per-Query IDF Statistics](#per-query-idf-statistics) to scope them to a subset, such as a single tenant.
