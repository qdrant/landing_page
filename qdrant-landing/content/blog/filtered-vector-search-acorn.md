...
short_description: "Weaviate, Elasticsearch, Vespa, Solr, and Qdrant all shipped ACORN. We benchmarked what it patches, what it costs, and where it breaks."
description: "Benchmark of filtered vector search strategies on Qdrant 1.18: ACORN vs filter-aware HNSW edges vs query planning, with a reproducible harness and receipts."
...
Between late 2024 and 2026, Weaviate, Elasticsearch, Vespa, Solr, and Qdrant adopted ACORN variants. ACORN repairs a filter-blind HNSW graph at search time. Qdrant's filter-aware HNSW repairs the graph at index time, by adding edges for indexed payload values, and did so before ACORN existed.

We benchmarked both approaches on the same engine and the same data, to see what each one catches that the other misses.

## What ACORN Actually Ships
...
ACORN-1 is the lighter variant: build a standard HNSW graph, then check neighbors of neighbors at search time when the filter removes too many direct ones. That patch is what the engines shipped, and every one of them gates it behind a selectivity check that decides when to stop using it.
...
That gap is what ACORN covers in Qdrant. Since [1.16](/blog/qdrant-1.16.x/), it has been one option behind the [query planner](/documentation/search/search/#query-planning), alongside full scan, retrieval straight from the payload index, and filterable HNSW.
...
We compare four strategies over the same data. The plain graph checks the filter during traversal and discards points that fail; it is the control, with filter-aware edges switched off. The second strategy forces ACORN on that same graph. The third is filterable HNSW.

The fourth is the planner on Qdrant's defaults, which stacks all three tools: filter-aware edges, ACORN, and the payload-index fallback. All four are Qdrant on one machine, configured four ways; no other engine was measured. Latency is mean server-side query time. Full methodology and reproduction instructions are in the [reproduction kit](https://github.com/qdrant-labs/acorn-filterable-hnsw-benchmark).
...
When filter-aware edges exist, they solve the problem earlier and cheaper. They hold 99.0% to 99.8% recall on the 10%, 1%, and correlated 10% filters at 1.0ms to 1.2ms. ACORN gets close on two of those filters, but it needs 3.5ms to 4.7ms. Even correlation only lifts the plain graph from 20.6% to 88.4%, which still does not catch the edges.

{{< figure src="/blog/filtered-vector-search-acorn/single-filters.png" alt="Bar chart of recall at hnsw_ef=64 on four single-field filters, with each bar's mean server-side latency, comparing plain graph, plain graph with ACORN, and filterable HNSW." caption="Recall@10 and each bar's mean server-side latency at hnsw_ef=64 on the single-field filters. Filter-aware edges hold the top recall at about 1ms; ACORN pays 3 to 4x that." width="100%" >}}

The 20% filter is the one case where ACORN finishes ahead of the edges, 98.9% against 94.8%. Its value groups are big enough that the main graph keeps them connected, so Qdrant skips extra edges for this field, and that connectivity alone does not deliver full recall at a small search budget. Recall still climbs from 62.9% to 94.8%, because edges built for the other fields densify the shared graph.
...
The tie breaks when the search budget rises. At `ef=512`, filterable HNSW reaches 91.2% recall at 4.9ms, while ACORN needs 20.1ms to reach 90.3%. Spending a bigger search budget on a filter-aware graph is cheaper than repairing a filter-blind one at search time.
...
When neither field has filter-aware edges, ACORN gets its clean win. That is the 4% intersection: both fields hold five values of 200k points, so neither earned extra edges, and the gap holds across the whole sweep, 99.6% against filterable HNSW's 92.5% at `ef=512`.
...
Planning matters before the graph fully collapses. On the 1% intersection, the planner sent 29 of the 500 queries to the graph and 471 to the payload index, reaching 100% recall at the 3.7ms mean shown before.

## Where ACORN Actually Wins

The pattern is simple: ACORN wins when Qdrant had no filter-aware edges for the queried field. That happened on two of the benchmark's seven payload fields, both with value groups too large for extra edges. On the five fields that did get edges, ACORN never beat them by a defensible margin.

That is ACORN's place in the system. It repairs the graph after filtering damages traversal, especially for oversized value groups and strict intersections. A [PostgreSQL study](https://arxiv.org/abs/2603.23710) shows the cost of that repair on a five-million-point OpenAI embedding set: 71.6K filter checks for ACORN at 1% selectivity against 2.6K for plain traversal.

Filtered-search quality hangs on three questions: what the engine builds before the query arrives, what the search budget buys during traversal, and when the engine stops traversing altogether. ACORN is one part of the middle answer.

Qdrant answers all three: filter-aware edges at index time, `ef` and opt-in ACORN at query time, and a planner that leaves the graph for the payload index when the filter is small enough to check directly. The [telemetry endpoint](/documentation/ops-monitoring/monitoring/) counts how many queries took each path, so you can see the split on your own workload.
