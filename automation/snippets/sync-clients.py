...
def main() -> None:
    ...
    fetch_parser = subparsers.add_parser(
        "fetch",
        help="""
            Fetch client libraries and put them in the clients/ directory.
    ...
    lock_parser.add_argument(
        "--rust",
        help="Set Rust client version. Format: branch=BRANCH_NAME or tag=TAG_NAME or rev=COMMIT_HASH",
        nargs="?",
        const="branch=master",
        metavar="KIND=REV",
    )
    lock_parser.add_argument(
        "--python",
        help="Set Python client version. Format: branch=BRANCH_NAME or tag=TAG_NAME or rev=COMMIT_HASH",
        nargs="?",
        const="branch=master",
        metavar="KIND=REV",
    )
...
def update_toml_spec(path: Path, spec: str) -> None:
    try:
        ...
        print(
            "Invalid format for --rust/--python. Use branch=BRANCH_NAME or tag=TAG_NAME or rev=COMMIT_HASH."
        )
        exit(1)
