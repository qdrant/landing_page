...
The all-in-one [`Dockerfile`](./docker/Dockerfile) and [`shell.nix`](./shell.nix) are provided. Run [`./docker.sh`](./docker.sh) to start a shell in the docker container.
...
Run the following then examine an output and confirm with `y`.
```bash
./migrate-snippet.py /path/to/snippet.md
```
...
```bash
 ./check.py build /snippets/your/new/snippet
```

(Optional) Start Qdrant and run the snippet.
```bash
./check.py run /snippets/your/new/snippet
```

(Optional) Write `test.py` and test it.
```bash
./check.py test /snippets/your/new/snippet
```
