...
In this article, we will try to understand which strengths of BM25 made it relevant for so many years, why current alternatives struggled to replace it, and how we can take the next step in the evolution of lexical search.

## Why BM25 was relevant

To understand why BM25 stayed relevant for so long, we must look into how it works.
...
So it works well for searching web pages, books, articles, etc.

However, would it work as well for modern search applications, such as RAG? Let's see.

The typical length of the document in RAG is much shorter than that in web search. In fact, even if we are working with the web-pages and articles, we prefer to split them into chunks so that
...
- How do you extract the importance information from the transformer?
...
Apart from the regular text tokens, the transformer model also has a special token called `[CLS]`. This token represents the whole sequence in the classification tasks, which is exactly what we need.
...
In our implementation, we use the `sentence-transformers/all-MiniLM-L6-v2` model, which gives a huge boost in the inference speed compared to the SPLADE models. But in practice, any transfotmer model can be used. 
...
Let's see how you can setup collection for hybrid search with BM42 and [jina.ai](https://jina.ai/embeddings/) dense embeddings.
...
Please note, that we didn't have an intention to make an exhaustive evaluation, as we are presenting a new approach, not a new model.
...
Please note, that both BM25 and BM42 won't work well on their own in production environment.
Best results are achieved with combination of Sparse and Dense embeddings in a hybrid approach.
In this scenario, those two models are complementary to each other.
Sparse model is responsible for the exact token matching, while Dense model is responsible for the semantic matching.
