...
![Filtered Vector Search: What ACORN Fixes, and What Fixes ACORN](https://qdrant.tech/articles_data/filtered-vector-search-acorn/preview/title.jpg)
...
## [Anchor](https://qdrant.tech/articles/filtered-vector-search-acorn/\#the-two-acorns) The Two ACORNs
...
## [Anchor](https://qdrant.tech/articles/filtered-vector-search-acorn/\#the-graph-qdrant-builds-instead) The Graph Qdrant Builds Instead

[Filterable HNSW](https://qdrant.tech/articles/filterable-hnsw/), which our co-founder Andrey Vasnetsov described in 2019, builds the repair into the index. When a payload field, the metadata attached to each point, is [indexed](https://qdrant.tech/documentation/manage-data/indexing/#payload-index), Qdrant adds extra HNSW edges between points that share a value in that field, so a filtered query keeps a connected graph to traverse. Qdrant gives those edges to payload fields at index time, and not every field earns them.
...
Qdrant builds those edges per payload field, never per combination, so an `AND` filter lands on an intersection that no single field’s edges cover. ACORN-1 covers that gap and pays at query time instead of build time. Qdrant’s [query planner](https://qdrant.tech/documentation/search/search/#query-planning) chooses automatically between ACORN, full scan, retrieval straight from the payload index, and filterable HNSW.

![Three panels of the same 12-point HNSW graph with a search path drawn in each. In the first, the path leaves a matching point and is blocked at its filtered-out neighbors. In the second, ACORN carries the path through two filtered-out neighbors to reach the other matching points. In the third, the path follows extra edges that filterable HNSW added between points sharing an indexed value.](https://qdrant.tech/articles_data/filtered-vector-search-acorn/two-repairs.svg)
...
## [Anchor](https://qdrant.tech/articles/filtered-vector-search-acorn/\#the-benchmark) The Benchmark
...
## [Anchor](https://qdrant.tech/articles/filtered-vector-search-acorn/\#single-filters-extra-edges-win) Single Filters: Extra Edges Win
...
This table compares the first three strategies. [`full_scan_threshold`](https://qdrant.tech/documentation/manage-data/indexing/#vector-index) tells Qdrant when a filtered result set is small enough to scan directly. The value is measured in kilobytes of vector data, and Qdrant skips the HNSW graph when the matching vectors fall below it.
...
![Bar chart of recall at hnsw_ef=64 on four single-field filters, with each bar's mean server-side latency, comparing plain graph, plain graph with ACORN, and filterable HNSW.](https://qdrant.tech/articles_data/filtered-vector-search-acorn/single-filters.png)

Bars show Recall@10; the label on each bar is its mean server-side latency. Extra edges hold the top recall at about 1ms; ACORN pays 3 to 5x that.
...
## [Anchor](https://qdrant.tech/articles/filtered-vector-search-acorn/\#why-some-payload-fields-get-no-extra-edges) Why Some Payload Fields Get No Extra Edges
...
## [Anchor](https://qdrant.tech/articles/filtered-vector-search-acorn/\#double-filters-the-intersection-gap) Double Filters: The Intersection Gap
...
![Recall versus server-side latency for four filtered-search strategies as hnsw_ef sweeps from 64 to 512.](https://qdrant.tech/articles_data/filtered-vector-search-acorn/ef-sweep.png)

Recall vs server-side latency on the 1% double filter alone, hnsw\_ef swept from 64 to 512.
...
## [Anchor](https://qdrant.tech/articles/filtered-vector-search-acorn/\#acorn-on-a-normal-collection) ACORN on a Normal Collection
...
## [Anchor](https://qdrant.tech/articles/filtered-vector-search-acorn/\#what-to-measure-on-your-own-collection) What to Measure on Your Own Collection

Measure recall for each filter shape you serve. Start with the ones most likely to break: values covering roughly a fifth of the collection or more, and `AND` combinations of them. [Facet counts](https://qdrant.tech/documentation/manage-data/payload/#facet-counts) show which values are that broad.
...
Compare the recall gain with the latency cost. A recovery like that 39.7% filter is what ACORN is for, while a point or two is worth taking only when the latency multiple is small. Set [`acorn.enable`](https://qdrant.tech/documentation/search/search/#acorn-search-algorithm) on the query paths whose filters earned it.
...
## [Anchor](https://qdrant.tech/articles/filtered-vector-search-acorn/\#further-reading) Further Reading
...
- [Filterable HNSW](https://qdrant.tech/articles/filterable-hnsw/): the 2019 article behind Qdrant’s extra edges.
...
To discuss your filtered-search setup, [get in touch](https://qdrant.tech/contact-us/).
...
We are sorry to hear that. 😔 You can [edit](https://qdrant.tech/github.com/qdrant/landing_page/tree/master/qdrant-landing/content/articles/filtered-vector-search-acorn.md) this page on GitHub, or [create](https://github.com/qdrant/landing_page/issues/new/choose) a GitHub issue.

On this page:

- [View as Markdown](https://qdrant.tech/articles/filtered-vector-search-acorn/index.md)
- [Edit on Github](https://github.com/qdrant/landing_page/tree/master/qdrant-landing/content/articles/filtered-vector-search-acorn.md)
