...
In this blog, we explore a combination of a dedicated vector search engine like Qdrant with an AI automation platform like n8n, moving beyond basic Retrieval-Augmented Generation (RAG) use cases. We'll show you how to use vector search for recommendations and big data analysis on ready-to-use n8n workflows.

## Setting Up Qdrant in n8n

To start using Qdrant with n8n, you need to provide n8n your Qdrant instance credentials. 
This can be done by adding them in the [credentials](https://docs.n8n.io/integrations/builtin/credentials/qdrant/#using-api-key) tab. 
Select `QdrantApi` from the list.
...
3. Navigate to the **API Keys** tab and copy your API key - the `API Key` in n8n.
...
Vector search ability to devise semantic similarity of objects is often used to deal with models' hallucinations, powering memory of Retrieval Augmented Generation-based applications.

Yet there's more than a "knowledge base" role to vector search. By exploring the concept of "dissimilarity," we unlock new possibilities. By measuring how similar data points are in a semantic vector space, we can also analyze their differences.

This combination of similarity and dissimilarity expands vector search to recommendations, discovery search, and large-scale unstructed data analysis. 

### Recommendations

When we're searching for new music, films, books, or food, it can be difficult for us to articulate exactly what we want. Instead, we often rely on discovering new content through comparison to examples of what we like or dislike.

The [Qdrant Recommendation API](https://qdrant.tech/articles/new-recommendation-api/) is built to make these discovery searches possible, by using positive and negative examples as anchors. It helps to find new relevant results based on your preferences.

#### Movie Recommendations
Imagine a home cinema night - you've already watched Harry Potter 666 times and crave a new series featuring young wizards. Your favorite streaming service repetitively recommends you all seven parts of the millennial saga. Frustrated, you turn to n8n to create an **Agentic Movie Recommendation tool**.
...
2. **Embedding Model** to vectorize movie descriptions. We'll use OpenAI `text-embedding-3-small`, but you can opt for any other suitable embedding model.
...
1. **Movie Data Uploader**: embed movie descriptions and upload them to Qdrant using the [Qdrant Vector Store Node](https://docs.n8n.io/integrations/builtin/cluster-nodes/root-nodes/n8n-nodes-langchain.vectorstoreqdrant). In the template workflow, the dataset is fetched from GitHub, but you can use any supported storage, for example [Google Cloud Storage](https://docs.n8n.io/integrations/builtin/app-nodes/n8n-nodes-base.googlecloudstorage).
2. **AI Agent**: use the [AI Agent Node](https://docs.n8n.io/integrations/builtin/cluster-nodes/root-nodes/n8n-nodes-langchain.agent) to formulate Qdrant Recommendation API calls based on your natural language-based requests. Choose an LLM as a "brain" and define a [JSON schema](https://docs.n8n.io/integrations/builtin/cluster-nodes/sub-nodes/n8n-nodes-langchain.toolworkflow/#specify-input-schema) for the recommendations tool powered by Qdrant. This schema lets LLM map your requests to the tool input format.
3. **Recommendations Tool**: a [subworkflow](https://docs.n8n.io/flow-logic/subworkflows/) that calls the Qdrant Recommendation API using the [HTTP Request Node](https://docs.n8n.io/integrations/builtin/core-nodes/n8n-nodes-base.httprequest). The agent will extract relevant and irrelevant movie descriptions from your chat message and pass them to the tool. The tool embeds them with `text-embedding-3-small` and uses the Qdrant Recommendation API to get movie recommendations which are passed back to the agent.
...
The possibility of mapping data to a vector space that reflects items' similarity and dissimilarity relationships provides a range of mathematical tools for data analysis. 
...
A practical example is worth a thousand words. Let's look at **Qdrant-based anomaly detection and classification tools**, which are designed to be used by [n8n AI Agent node](https://docs.n8n.io/integrations/builtin/cluster-nodes/root-nodes/n8n-nodes-langchain.agent) for data analysis automation.

To make it more interesting, this time we’ll focus on image data.

#### Anomaly Detection Tool

One of the definitions of "anomaly" comes intuitively after projecting vector representations of data points into a 2D space - Qdrant webUI provides this functionality. Points that don't belong to any clusters are more likely to be anomalous.
...
*There are three workflows: (1) Uploading images to Qdrant (2) Setting up cluster centres and thresholds (3) Anomaly detection tool itself.*
...
2. **Payload Index**: adds a [payload index](https://qdrant.tech/documentation/concepts/indexing/#payload-index) on the `crop_name` payload (metadata) field. This field stores crops class labels and indexing it improves the speed of filterable searches in Qdrant. It changes the way a vector index is constructed, adapting it for fast vector search under filtering constraints. For more details, refer to this [guide on filtering in Qdrant](https://qdrant.tech/articles/vector-search-filtering/).
...
| **Method**            | **Steps**                                                                                                                                                                                                                                                                                                                                                     |
|------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Medoids**           | 1. Sample labelled cluster points from Qdrant. <br> 2. Compute a **pairwise distance matrix** for the cluster using Qdrant's [Distance Matrix API](https://qdrant.tech/documentation/concepts/explore/?q=distance+#distance-matrix). This API helps with a scalable cluster analysis and data points relationship exploration. Learn more in [this article](https://qdrant.tech/articles/distance-based-exploration/). <br> 3. For each point, calculate the sum of its distances to all other points. The point with the smallest total distance (or highest similarity for COSINE distance metric) is the medoid. <br> 4. Mark this point as the cluster representative. |
...
4. Calculate the candidate's similarity to each cluster representative. The image is flagged as anomalous if the similarity is below the threshold for all clusters (outside the cluster borders). Alternatively, you can check if it's anomalous to a particular cluster, for example, cherries one.
...
This tool is adaptable to these use cases and, when combined with n8n integrations, has a potential to become a production-level business solution.
...
The KNN method labels a data point by analyzing its classified neighbors and assigning this point the majority class in the neighbourhood. This approach doesn’t require all data points to be labeled, a subset of labeled examples can serve as anchors to propagate labels across the dataset. Qdrant is well-suited for this task, offering fast neighbor searches with filtering capabilities.
...
2. **Embedding Model**: as for anomaly detection, we'll use [Voyage AI multimodal embedding model](https://docs.voyageai.com/docs/multimodal-embeddings).
...
2. **Fetch neighbours:** retrieves the K closest labelled neighbors from Qdrant.
...
Though classification seems like a task which was solved in machine learning decades ago, it's not so trivial to deal with in production. Issues like data drift, shifting class definitions, mislabeled data and fuzzy difference between classes create unexpected problems, which require continuous adjustements of classifiers. Vector Search can be an unusual but effective solution, interesting due to its scalability.

#### Live Walkthrough

To see how n8n agents use these tools in practice, and to revisit the main ideas of "*Big Data Analysis*" section, watch our integration webinar
