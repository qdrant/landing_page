...
It is possible to use asynchronous HTTP API by using `qdrant_client.http.api_client.AsyncApiClient`. The asynchronous gRPC API if more efficient than the HTTP API, which is why we recommend you use it and we will be using it here. 
