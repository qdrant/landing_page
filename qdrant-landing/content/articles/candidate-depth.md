...
As soon as a collection has to fit somewhere real, retrieval tuning is no longer only about relevance. Latency, memory, disk, and build time meet at the same controls: how many candidates you retrieve, how hard the index works to find them, and what the collection costs to keep in RAM.
...
Score the union of your two candidate lists as if it were ordered perfectly. That is the best nDCG@10 any ranking of those candidates could reach. nDCG@10 grades the top 10 results and gives more credit to relevant documents near the top; [choosing a metric](/articles/before-tuning-a-qdrant-collection/#choose-a-metric-before-you-tune) covers when to use it. Raising the prefetch `limit` moves that best-possible score a long way.
...
Depth costs less latency than a model call, which argues for setting it generously. In one fused `query_points` request, going from `limit=10` to `limit=500` added 40% to 45% to the median: 2.14 ms to 3.06 ms on SciFact, and 2.77 ms to 3.94 ms on DBPedia-entity.
...
The sweep was flat on these corpora. Moving through 16, 64, 128, and 512 at depth 200 changed the fused score by at most 0.0022 on any of the five, and union recall by at most 0.0040. On SciFact, the results at 128 and 512 are byte-identical.
...
Once the graph stops saturating, `hnsw_ef` becomes your primary recall-against-latency knob. Point count is only one input. Vector distribution, filters, and query difficulty also move the saturation point, so no collection size tells you which side you are on.
...
Recall starts at 0.986 and has almost nowhere to go. On a fused query at prefetch `limit` 200, raising `hnsw_ef` from 16 to 512 added between 4% and 49% to median latency across the five corpora, for at most 0.0022 of nDCG@10. Here, the larger search budget is close to pure cost.
...
The gap was large on all five corpora. [Reranking](/articles/when-a-reranker-is-worth-it/) tests whether another ranking stage can collect it, and what that stage costs.
