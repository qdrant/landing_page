...
draft: false
slug: minicoil-en-es
short_description: "miniCOIL EN-ES is a bilingual sparse neural retriever where English and Spanish words with a shared meaning land in one concept slot."
description: "How we built miniCOIL EN-ES with Pento AI: a bilingual sparse neural retriever matching English and Spanish by concept in one index, no translation step."
preview_image: /blog/minicoil-en-es/cover.png
...
Our [miniCOIL v1](https://qdrant.tech/articles/minicoil/) sparse neural retriever article ended up with a promise: to keep working on this sparse neural retriever in-depth, improving the model's quality, and in-width, *"extending it to other dense encoders and to languages beyond English."*
...
{{< figure src="/blog/minicoil-en-es/00-hero-shared-slot.png" alt="miniCOIL EN-ES: the English word dog and the Spanish word perro both write into the same block of cells in one sparse vector." caption="miniCOIL EN-ES: \"dog\" and \"perro\" land in the same slot of a sparse vector." width="90%" >}}

We're sharing the design, the detours, and a first set of results.  

We'd also love your feedback: whether you'd use something of the kind, whether the approach works for you. If so, we'd scale the model's vocabulary and training, and let's see what comes next: the idea is easily transferable to other languages.

## Same Word, Different Language

Word-based sparse retrieval has a limitation that no amount of relevance-based weighting alone can fix: it is based on matching exact terms: *"dog"* and *"perro"* mean the same thing, but to BM25 and co, they aren't, they are two different tokens in two different postings lists.
...
It works, but it puts a translation model in the hot path of a query, adds an additional dependency, and leads to management of a separate index (or a separate translated copy of the corpus) per language pair.

#### Dense Multilingual Retrieval

Multilingual dense encoders solve cross-lingual matching by construction: *"She won a prize"* and *"Ella ganó un premio"* land close together in embedding space. (Well, they *should* land so: read on language bias in multilingual dense retrievers in our [recent blog](https://qdrant.tech/blog/shift-multilingual-rag/).)
...
So we don't want to translate and we don't want to limit ourselves to dense retrievers.

Instead, we want to reuse the miniCOIL recipe: take the wisdom of information retrieval decades folded into the BM25 formula and throw on top a small learned component that adds meaning-in-the-context.  
...
{{< figure src="/blog/minicoil-en-es/00-how-crosslingual-is-done.png" alt="Three ways to do cross-lingual retrieval: translate then search, dense multilingual encoders, and miniCOIL EN-ES concept matching." caption="How cross-lingual full-text retrieval is usually solved: translate the query, or use dense multilingual encoders. Sparse neural retriever \"miniCOIL EN-ES\" takes a third route: concept matching." width="90%" >}}
...
> Concepts are not specific to English and Spanish, the approach is transferable to other languages. More about this further in the blog.
...
{{< figure src="/blog/minicoil-en-es/01-concept-slots.png" alt="Two sparse vectors compared: miniCOIL v1 has one block per English word and no slot for perro; miniCOIL EN-ES has one shared block per bilingual concept plus a BM25 fallback cell." caption="One slot per concept: in miniCOIL v1 a Spanish query has nothing to match against; in miniCOIL EN-ES \"dog\" and \"perro\" resolve to the same concept and write into the same cells." width="90%" >}}
...
* We also cap cluster size: Louvain's resolution parameter controls how granular the communities are (higher resolution favors smaller, tighter ones), so on any cluster that grows too large we re-run Louvain with the resolution bumped up, until a concept word count fits under the cap.
...
MUSE lists translations in frequency order, so we keep only the **top-2 translations per source word** before building the graph. Combined with a limit of a maximum cluster size of 20, this yields about 80k clean concepts, median size 2 (a typical concept is literally just a word and its translation, like `{cat, gato}`) and 99th percentile size 9.

{{< figure src="/blog/minicoil-en-es/02-muse-graph.png" alt="Left: a bipartite English-Spanish dictionary graph with tight communities forming concepts. Right: the polysemous Spanish word bajo stitching many unrelated words into one giant cluster." caption="Concepts are communities in a bilingual dictionary graph. Polysemous words like Spanish \"bajo\" stitch unrelated terms into hairballs; keeping the top-2 translations per word and capping cluster size keeps concepts clean." width="90%" >}}

#### 80k Concepts Are 30 Times Too Many

For verifying if the approach works, keeping 80,000 concepts and training a layer per each seemed unnecessary (for example, we could live without the {mockingjay, sinsajo} one).

To trim the concept vocabulary, we streamed English and Spanish Wikipedia, split articles into sentences, and counted how many sentences contain each concept in each language, keeping only concepts with enough evidence on both sides. This leaves roughly 12,000 concepts with enough sentences for a per-concept head to learn to light up the right sense region of its concept, in whichever of the two languages the match comes.

For the released model we went even further and kept out of 12k only concepts that retrieval queries in mMARCO actually use. Ranking concepts by how many of query-word occurrences they catch, the top 2,398 concepts grab 90% of everything the full 12k vocabulary would.  
...
All sizes retrieved the parallel counterpart 100% of the time, with average cross-lingual cosine similarity of 0.91 to 0.93. Hence, for convenience (*including availability of free inference on Qdrant Cloud*), we chose the smallest backbone option: [`intfloat/multilingual-e5-small`](https://huggingface.co/intfloat/multilingual-e5-small), which we refer to as `mE5-small`: 384 output dimensions, 118M parameters.
...
{{< figure src="/blog/minicoil-en-es/04-five-point.png" alt="Quadrant diagram of five-point sampling: an anchor sentence in the center, same-language and cross-language positives pulled toward it, same-language and cross-language negatives pushed away." caption="Five points per training example: same- and cross-language positives and negatives around each anchor, with \"close\" and \"far\" decided by the teacher encoder." width="90%" >}}
...
The final model has 2,398 heads, ~7.4M parameters, is about 6% of the backbone encoder and ships as a ~29.5 MB safetensors file.
...
1. Tokens that don't resolve to a trained concept are weighted by the plain BM25 fallback, which uses the same English tokenizer, English stemmer and default parameters as FastEmbed's `Qdrant/bm25`. We reserve for OOV tokens their own region of the sparse vector.

2. Raw `tanh` outputs, that is, the 8 values of a miniCOIL EN-ES concept block, come out much smaller than a typical BM25 word weight (their vector norm averages ~0.15), so a concept match would *lose* compared to a BM25-based match on the initial term.  
...
3. We used a cheap dirty fix of dropping the query language's own stopwords on the query side, with documents untouched. For future experiments, we should find a way to make the BM25 fallback bilingual, not always-English: stopwords, tokenization and stemming behave differently per language.
...
The fallback is gated by stopword lists, checked for both the token and its lemma.  
...
On our validation split, turning this fallback on buys roughly +0.01 MRR@10 in every retrieval direction, most in Spanish and cross-lingual, while recall stays put: it doesn't retrieve more, it lets the concept heads score passages that were already being found. It is on by default in the published model, and all results that follow use it.
...
Gains could be even larger, as while the BM25 ES→ES baseline is configured with Spanish in mind, miniCOIL's current BM25 fallback is non-flexible, processing any input with an English stemmer, which makes it only suitable for matching named entities or numbers. A per-language fallback could improve the metrics.
...
miniCOIL EN-ES trails translate-then-BM25 on top-10 ranking in both cross-lingual directions, however, recall@100 goes for both the other way: the concept mechanism finds the relevant passages across the language border, while translated exact matching orders the top better.

However, two caveats make this result look like a successful one: translate-then-BM25 has to use a translation model in the hot path of every query, while miniCOIL EN-ES retrieves cross-lingually from one index, no translation needed, on a heavily pruned concept vocabulary.  
Moreover, current miniCOIL EN-ES's own BM25 fallback runs the English stemmer on all inputs, Spanish included, so part of its gap can be called configuration debt.

## Takeaways

This blog is about a preliminary experiment. The model behind these tables trains 2,398 concepts out of a vocabulary of roughly 12,000, on at most 800 sentences per concept and language taken from a small slice of Wikipedia.  
Within that scope, evaluation provides us with the evidence of the model's potential.
...
**BM25 underneath.** Everything out of vocabulary falls back to BM25, so the model degrades to a known baseline. However, in a multilingual setting, configuring this fallback gets trickier, so here might hide a large room for improvement.
