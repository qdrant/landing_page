...
As we've already said in our [blog](https://medium.com/@qdrant/what-is-hybrid-search-2a0c30d0f3d2), there is no single definition of hybrid search. 
In this tutorial we're covering the case with a combination of dense and (sparse embeddings)[/articles/sparse-vectors/]. 
The former one refers to the embeddings generated by well-known neural networks like BERT, while the latter one is more similar to a traditional full-text search approach.
...
<aside role="status">
There are versions of this tutorial that use only dense models with <a href="/documentation/tutorials/neural-search/">SentenceTransformers</a> and <a href="/documentation/tutorials/neural-search-fastembed/">Fastembed</a>.
</aside>
...
Methods like `get_fastembed_vector_params` and `get_fastembed_sparse_vector_params` help you to get the corresponding parameters for the models you are using.
Such parameters might include e.g. vector size and distance function.
...
neural_searcher = HybridSearcher(collection_name="startups")
