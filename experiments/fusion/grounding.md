...
| Across `hnsw_ef` 16 to 512 at depth 200, the default arm moves at most 0.0022 and union recall at most 0.0007, on all five corpora | Same file, `hnsw_ef_sweep` spans |
...
| Prefetch `limit` 10 to 500 costs 40% to 45% more median latency; `hnsw_ef` 16 to 512 costs 9% to 49% for at most 0.0022 of quality | `study/e3_latency.json`, one fused `query_points` per request, randomized across settings, warmup discarded, 100 queries times 3 repeats. Apple M5 Pro, single shard, no concurrent load |
...
| A cross-encoder beat a tuned fusion on one corpus of five, by +0.090 on DBPedia, and lost on the other four | `study/e4_reranking.json`. Each split selects both the reranker configuration and fusion arm on one query half, then compares them on the other; DBPedia clears on 100% of splits, the rest on 0% |
| Against the RRF default instead, reranking looks like a win on three of five | Same file, `vs_rrf_default` against `vs_best_fusion_arm`. The gap between those two columns is why both are reported |
| The deficit grows with candidate count where the reranker is weaker than the fusion, and the gain grows where it is stronger | Same file, `configurations` across counts 10 to 200 |
| Cross-encoder throughput on CPU: 64 to 212 docs/sec for MiniLM-L6, 34 to 117 for L12, 16 to 45 for bge-reranker-base | `study/rerank/throughput.json`. The range is document length across corpora. Apple M5 Pro, one process, 15 threads |
| `bge-reranker-base` is 8x the size of MiniLM-L12 and is beaten by a MiniLM on four of five corpora | `study/e4_reranking.json`, model sizes from `TextCrossEncoder.list_supported_models()` |
