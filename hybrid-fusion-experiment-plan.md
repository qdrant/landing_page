...
| The audit's bootstrap interval with no way to produce its input | The audit now carries the step that turns two configurations' results plus labels into per-query gains, with its own ten-line `nDCG@10` so the reader installs nothing. `test_article_ndcg.py` holds it to `pytrec_eval` on all 300 SciFact queries. Without it the series is not actionable without leaving the page |
