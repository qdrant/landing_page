...
#### Dissimilarity: using only negative examples
...
Recommendations can also be done in [groups](../search/#grouping-api), where the results are grouped by a given field. This is useful for recommendation systems that need to group results by genre, for example.
