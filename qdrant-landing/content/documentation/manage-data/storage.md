...
aliases:
  - ../storage
  - /documentation/manage-data/storage/
---
...
- Set up `memmap_threshold` option. This option will set the threshold after which the segment will be converted to memmap storage.
  ...
  1. You can set the threshold globally in the [configuration file](/documentation/configuration-ops/configuration/). The parameter is called `memmap_threshold` (previously `memmap_threshold_kb`).
...
You can specify the desired type of payload storage with [configuration file](/documentation/configuration-ops/configuration/) or with collection parameter `on_disk_payload` during [creation](/documentation/manage-data/collections/#create-collection) of the collection.
