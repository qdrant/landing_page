...
Qdrant provides a subtype of relevance feedback-based retrieval, where feedback is given by any model (relevance oracle) in a granular fashion, i.e., it rescores top retrieved results by their relative relevance to the query.  
A detailed overview of relevance feedback methods is in this article ["Relevance Feedback in Information Retrieval"](https://qdrant.tech/articles/search-feedback-loop).

To use relevance feedback-based retrieval, two components are required:

1. A collection of vectors to search through.
2. An oracle to determine relevance of search results.
...
3. Through analyzing the Feedback Score for the top results, figure if the feedback model agrees with a retriever, or retrieval can be improved.
...
The feedback model considers the second result with ID 222 to be the most relevant, which is a discrepancy with retriever's ranking.  
Hence, there's a potential that this feedback can help make the next iteration of retrieval better.

---

To leverage the feedback in search across the entire collection, Qdrant provides a special kind of querying interface.  
It needs:

1. The original query (`target`).
2. A short list of initial retrieval results, and their relevance score (`feedback`).  
   Each feedback item is presented by:
...
Internally, we will combine the feedback list into pairs, based on the relevance scores, and then use these pairs in a formula that modifies vector space traversal during retrieval (change the strategy of retrieval). This new relevance feedback-based retrieval considers not only the similarity of candidates to the query but also to each feedback pair.  

For a more detailed description on how it works, check out the [Relevance Feedback](TO DO: insert link here) article.

### Naive strategy

For now, the `naive` strategy is the only one available.
<details>
