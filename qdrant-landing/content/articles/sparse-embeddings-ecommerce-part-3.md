---
title: "Fine-Tuning Sparse Embeddings for E-Commerce Search, Part 3: Evaluation, Hard Negatives, and Results"
short_description: "Index products in Qdrant, benchmark against BM25, and push further with hard negative mining."
description: "Part 3 of a 4-part series. Index sparse vectors in Qdrant, evaluate with nDCG and MRR, implement ANCE hard negative mining, and analyze the 28% improvement over BM25 on Amazon ESCI."
...
We have a trained SPLADE model sitting on a Modal volume. Now comes the question that matters: is it actually better? In this article, we'll index products into Qdrant, run retrieval benchmarks, implement hard negative mining, and dig into what the model learned.
...
- **`on_disk=True`** keeps the inverted index on disk instead of RAM. SPLADE vectors average 200 active terms — across millions of products, this adds up. Requires SSD for acceptable latency.
...
│                    ANCE Training Loop                    │
│                                                          │
│   1. Index products into Qdrant with current model       │
│   2. For each query, retrieve top-K products             │
│   3. Filter to non-relevant products = hard negatives    │
│   4. Train on (query, positive, hard_negatives)          │
...
Sparse retrieval keeps mining cheap — sub-millisecond per query in Qdrant. For 100K queries, the mining step takes seconds, not minutes. Payload filters exclude known positives so you don't accidentally treat a relevant product as a negative.
...
| Stage | Latency | Notes |
|-------|---------|-------|
| **Query encoding** (SPLADE) | 10-20ms | The transformer forward pass — this is the bottleneck |
| **Sparse retrieval** (Qdrant) | <1ms | Inverted index lookup, even with millions of products |
| **Total** | 10-20ms | Fast enough for real-time search |
